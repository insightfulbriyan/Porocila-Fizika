...
ax[1].set(xlim=(0, time_limit), ylim=(-2, 2), ylabel='hitrost ($\\frac{\\text{m}}{\\text{s}}$)', title="$v(t)$")
ax[1].grid()
...
ax[2].set(xlim=(0, time_limit), ylim=(-20, 20), ylabel='pospešek ($\\frac{\\text{m}}{\\text{s}^2}$)', title="$v(t)$")
ax[2].grid()
...
# print(f"Frekvenca: {frekvenca}; krožna frekvenca: {2*np.pi*frekvenca}")
# print(f"Amplituda odmika: {np.max(amplituda_x*np.sin(2*np.pi*frekvenca*(x[0] - 0.0852))[:495])}")
# print(f"Napaka v odmiku {np.average(np.abs(amplituda_x*np.sin(2*np.pi*frekvenca*(x[0] - 0.0852))[:495] - x[4][:495] + np.mean(x[4][:495])))}")
# print(f"Amplituda hitrosti: {np.max(dxdy(amplituda_x*np.sin(2*np.pi*frekvenca*(x[0] - 0.0852)))[:495])}")
# print(f"Napaka v hitrosti {np.average(np.abs(dxdy(amplituda_x*np.sin(2*np.pi*frekvenca*(x[0] - 0.0852)))[:495] - x[5][:495]))}")
# print(f"Amplituda pospeška: {np.max(dxdy(dxdy(amplituda_x*np.sin(2*np.pi*frekvenca*(x[0] - 0.0852))))[:495])}")
# print(f"Napaka v pospešku {np.average(np.abs(dxdy(dxdy(amplituda_x*np.sin(2*np.pi*frekvenca*(x[0] - 0.0852))))[:495] - x[6][:495]))}")
